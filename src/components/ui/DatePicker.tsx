...
// ── Stable sub-components (defined outside to avoid recreation on each render) ──
...
function MonthNav({ month, year, onPrev, onNext }: MonthNavProps) {
  return (
...
function DayGrid({ days, year, month, selectedDate, cellMin, onSelect }: DayGridProps) {
...
function TodayBtn({ onClick }: TodayBtnProps) {
  return (
    <button
      type="button"
    ...
    <div className={`relative ${className}`}>
      ...
      {mounted && isOpen && (
        isMobile ? (
          ...
          createPortal(
            <>
              {/* Backdrop — sem blur para evitar faixa */}
              <div
